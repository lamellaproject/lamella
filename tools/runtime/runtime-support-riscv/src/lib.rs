...
core::arch::global_asm!(
    ...
    ".type   lamella_thread_switch_riscv, @function",
    ...
    "lw   ra,  52(a1)",
    "ret",
    ".size   lamella_thread_switch_riscv, . - lamella_thread_switch_riscv",
);
...
unsafe extern "C" {
    /// The AMBIENT `System.String` descriptor, one word, laid by the AOT image (`riscv32.rs`, under
    /// the shared `resolver::STRING_TYPEDESC_SYMBOL` name the ARM emitter also uses). 0 when that
    /// build could not name `System.String`.
    ///
...
/// Allocate a `[desc][len: u32][u16 units ...]` string of `units` code units and return the OBJECT
/// pointer -- the word after the header, which is what every string reader is handed.
///
/// **THE ONE WAY THIS ARCHIVE MAKES A STRING, AND THAT IS THE POINT RATHER THAN A TIDINESS.** Three
/// seams each bump-allocated their own with no header word at all, so a virtual call through any
/// string they produced read the previous allocation's tail as a vtable pointer. Extracting the
/// allocation means a seam added later cannot get it wrong, because reserving a header is no longer
/// something a seam does.
///
/// **THE HEADER IS THIS FUNCTION'S JOB HERE AND THE ALLOCATOR'S ON ARM**, and that asymmetry is the
/// targets' allocator convention rather than a divergence: `lamella_gc_alloc_impl` in the thumb crate
/// reserves and writes the word itself, while every RISC-V provider returns the raw BLOCK and the
/// emitted `Alloc` lays `[block+0] = descriptor` and hands back `block + 4`. This does exactly what
/// the emitted `Alloc` does, so a seam string and an `Alloc`-made object have one shape.
///
...
/// an image degrades to empty text rather than dispatching through a zero. That path is unreachable
/// once the emitter refuses such a build on the host, and it is kept because a degradation that is
/// dead is still cheaper than a fault that is not.
fn alloc_string(units: u32) -> *mut u32 {
    let type_desc = unsafe { core::ptr::read_volatile(&raw const __lamella_string_typedesc) };
    if type_desc == 0 {
        return core::ptr::null_mut();
    }
    let Some(bytes) = units
        .checked_mul(2)
    ...
    unsafe {
        core::ptr::write(block, type_desc);
        let obj = block.add(1);
        core::ptr::write(obj, units);
        obj
    }
}
