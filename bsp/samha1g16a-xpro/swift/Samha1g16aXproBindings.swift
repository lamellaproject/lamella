// GENERATED by lamella-bsp-gen from bsp/samha1g16a-xpro/board.toml + csp/samha1/ strata -- DO NOT EDIT.
// Regenerate: cargo run -p lamella-bsp-gen -- gen-family . samha1
//
// The samha1g16a-xpro board bindings as Swift constants, name/value-identical to Samha1g16aXproBindings.g.cs:
// every value below is a generation-time literal derived from the strata. Board truth
// lives in board.toml, never here.

public enum Samha1g16aXproBindings {
    // -- identity --
    public static let BOARD_MODEL: UInt16 = 50
    public static let BOARD_VENDOR: String = "Microchip"
    public static let CARRIER_USB_VID: UInt16 = 0x03EB

    /// driver family: which REGISTER MAP is behind a role, as `<chip family>-<block>`. The
    /// role's `KIND` says what the application asked for -- a uart, an spi -- and two
    /// peripherals of the same kind can share no register at all, so a consumer that selects a
    /// driver at run time needs both: KIND is what was asked for, this is what the silicon is.
    /// Neither alone is enough. One SERCOM block serves uart, spi and i2c, so the block does not
    /// name a driver; and a uart is a different register map on every family, so the kind does
    /// not either. Derived from the bound instance's block, so it cannot be transcribed wrongly.
    public static let VCP_DRIVER_FAMILY: String = "samha1-sercom"

    // -- VCP: a sercom-usart binding descriptor --
    public static let VCP_SERCOM_BASE: UInt32 = 0x42000C00
    public static let VCP_IRQ: UInt32 = 10
    public static let VCP_GCLK_CLKCTRL_VALUE: UInt32 = 0x4015
    public static let VCP_APBC_MASK: UInt32 = 0x8
    public static let VCP_PMUX_TX_REG: UInt32 = 0x4100443D
    public static let VCP_PMUX_TX_MASK: UInt32 = 0xF0
    public static let VCP_PMUX_TX_VALUE: UInt32 = 0x20
    public static let VCP_PMUX_RX_REG: UInt32 = 0x4100443E
    public static let VCP_PMUX_RX_MASK: UInt32 = 0xF
    public static let VCP_PMUX_RX_VALUE: UInt32 = 0x2
    public static let VCP_PINCFG_TX_REG: UInt32 = 0x4100445B
    public static let VCP_PINCFG_RX_REG: UInt32 = 0x4100445C
    public static let VCP_TXPO: UInt32 = 0
    public static let VCP_RXPO: UInt32 = 1
    public static let VCP_BAUD_115200_OSC8M_8MHZ: UInt32 = 0xC505

    // -- on-board devices: PORT group base + pin index + mask --
    public static let LED0_PORT_BASE: UInt32 = 0x41004400
    public static let LED0_PIN: UInt32 = 0
    public static let LED0_MASK: UInt32 = 0x1
    public static let LED0_ACTIVE_LOW: UInt32 = 1
    public static let BUTTON0_PORT_BASE: UInt32 = 0x41004480
    public static let BUTTON0_PIN: UInt32 = 3
    public static let BUTTON0_MASK: UInt32 = 0x8
    public static let BUTTON0_ACTIVE_LOW: UInt32 = 1
}
