//! The peripheral-table harvester: parses a `docs/peripheral-tables/*.toml` register-fact table
//! (the family's single source of truth, [[design-single-source-peripheral-facts]]) and emits
//! generated per-language bindings under `bsp/<chip>/<lang>/`, so no driver hand-transcribes a
...
impl<'a> ValueCursor<'a> {
    ...
    fn parse(&mut self) -> Result<RawValue, String> {
        self.skip_space();
        match self.peek() {
...
"#;
    ...
    fn emits_resolved_literals_and_shifted_masks() {
        ...
        let emitted = emit_csharp(&table, "docs/peripheral-tables/spi-rp2350.toml").expect("emits");
...
"#;
    ...
    fn emits_adc_facts_channels_and_calibration() {
        let table = parse(SAMPLE_ADC).expect("parses");
        let out = emit_csharp(&table, "docs/peripheral-tables/adc-rp2350.toml").expect("emits");
