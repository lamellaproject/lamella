...
#[cfg(all(feature = "profile-full", not(feature = "generics")))]
compile_error!(
    "profile-full must carry `generics`: it is the tier that offers the whole surface, and a \
     profile omitting it refuses `List<int>` while calling itself full"
);
#[cfg(all(feature = "profile-no-float", not(feature = "generics")))]
compile_error!(
    "profile-no-float must carry `generics`: it is profile-full without floating point, and \
     dropping float is not a reason to drop a capability that has nothing to do with it"
);
...
/// The message types and rules of the LIVE debug agent, re-exported from the crate that owns them:
/// read and write the target's memory **while a deployed program is still running**.
///
/// The agent lives in `lamella-debug-agent` rather than here, and the reason is what an
/// ahead-of-time compiled image can link: such an image is debugged over the same wire and cannot
/// depend on this crate, because this crate is the interpreter. The agent names no runtime, so both
/// tiers serve a live request through that one implementation.
pub use lamella_debug_agent::live;

/// The address span the LIVE debug agent may read and write, as a firmware stores it: a pair of
/// atomics in this crate's `.bss`.
///
/// The storage is here and the RULE is the agent's, and the split is not tidiness: an
/// ahead-of-time compiled image links an archive with no writable-data segment model, where a
/// static like this resolves into `.text`, so the compiled tier keeps the same two numbers in its
/// caller-provided RAM region instead. A firmware has a `.bss` and this is the cheapest place for
/// it. Both hand the pair to the same [`lamella_debug_agent::serve_live_frame`], which stores
/// nothing.
...
pub fn set_live_window(base: u32, len: u32) {
    ...
    let Some((base, len)) = lamella_debug_agent::checked_window(base, len) else { return };
...
mod tests {
    ...
    /// The one rule in the LIVE agent's neighbourhood that is THIS crate's rather than the agent's:
    /// whether a host is TOLD the agent is there. The agent's own suite lives with the agent, in
    /// `lamella-debug-agent`; what stays here is the JOIN, because the bits it joins are the serve
    /// tier's and an AOT image composes a different advertisement from the same window.
    #[cfg(feature = "baked-image")]
    mod live_agent_advertisement {
        use super::super::deploy_caps_with;
        use lamella_wire::Capabilities;
