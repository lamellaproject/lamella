//! The bytecode interpreter for the first-light subset.
...
/// model, Numbers -- so a bignum is the eventual home; deferred past first light). A
/// string constant is not in the first-light interpreter subset.
...
/// arbitrary types (the reflected `__add__`/`__radd__` protocol) arrives with the full
/// object model.
///
...
/// -- bignum promotion is a first-light deferral. `//` floors toward negative infinity
...
/// `+x == x`, `-x`, `~x == -x - 1`); other types are a `TypeError`.
...
/// operators `<`/`<=`/`>`/`>=` have no default and raise `TypeError`.
...
/// [`MAX_CALL_DEPTH`].
///
/// First-light GC note: each activation roots only its own [`Frame`]; a collection that
...
/// is the follow-up when the object model grows allocation inside calls.
fn exec(
    code: &CodeObject,
    functions: &[CodeObject],
    args: &[Value],
    ...
    loop {
        ...
        match op {
            ...
            Op::Binary(binop) => {
                let rhs = frame.pop()?;
                let lhs = frame.pop()?;
                frame.push(binary(binop, lhs, rhs)?);
            }
            Op::Unary(unop) => {
                let value = frame.pop()?;
            ...
            Op::Compare(cmpop) => {
                let rhs = frame.pop()?;
                let lhs = frame.pop()?;
                frame.push(compare(cmpop, lhs, rhs)?);
            }
            Op::PopTop => {
                frame.pop()?;
            }
            ...
            Op::Call(argc) => {
                ...
                let index = callee.as_function_index().ok_or(Trap::TypeError)?;
                let callee_code = functions.get(index as usize).ok_or(Trap::Malformed)?;
                let result = exec(callee_code, functions, &call_args, model, depth + 1)?;
                frame.push(result);
            }
            Op::Return => {
                return frame.pop();
...
mod tests {
    ...
    /// Builds a first-light code object from the minimal fields the interpreter reads,
    ...
    /// The iterative `fib` of the first-light plan:
    /// ```python
    /// def fib(n: int) -> int:
    ///     a = 0
    ///     b = 1
