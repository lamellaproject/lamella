...
mod tests {
    ...
    fn an_undefined_symbol_renders_the_input_that_would_have_defined_it() {
        ...
        assert!(rendered.contains("allocation"), "{rendered}");
    ...
    /// The rendering is a RENDERING, not a string that happens to contain the symbol.
    ///
    /// The derived `Debug` contains the symbol too and nothing else, so a `Display` that fell back to
    /// it would satisfy every content assertion above while handing a reader the unactionable form
    /// the `Display` exists to replace. Asserting the content without asserting the difference would
    /// leave that substitution invisible.
    #[test]
    fn the_rendered_failure_is_not_merely_the_debug_form() {
        let seam = LinkError::UndefinedSymbol(String::from("lamella_gc_alloc"));
        let rendered = alloc::format!("{seam}");
        let debug = alloc::format!("{seam:?}");
        assert_ne!(rendered, debug, "Display must not fall back to Debug");
        assert!(
            !debug.contains("runtime-support archive"),
            "the Debug form is the unactionable one this exists to replace: {debug}"
        );
    }
