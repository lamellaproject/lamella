// A System.Device.Gpio driver for the Microchip SAMDA1, PA00..PA31 + PB00..PB31.
using System.Device.Gpio;
using Lamella.Generated;
using Lamella.Hardware;

public sealed class Samda1GpioDriver : GpioDriver
{
    ...
    {
        Mmio.Write32(GroupBase(pinNumber) + Samda1PortLayout.DIRCLR_OFF, PinMask(pinNumber));
    ...
    protected override void SetPinMode(int pinNumber, PinMode mode)
    {
        ...
        {
            Mmio.Write8(PinCfgAddress(pinNumber), (byte)Samda1PortLayout.PINCFG0_INEN);
            Mmio.Write32(group + Samda1PortLayout.DIRSET_OFF, mask);
            return;
        }

        Mmio.Write32(group + Samda1PortLayout.DIRCLR_OFF, mask);
        if (mode == PinMode.InputPullUp)
        {
            Mmio.Write32(group + Samda1PortLayout.OUTSET_OFF, mask);
            Mmio.Write8(PinCfgAddress(pinNumber),
                (byte)(Samda1PortLayout.PINCFG0_INEN | Samda1PortLayout.PINCFG0_PULLEN));
            return;
        }
        if (mode == PinMode.InputPullDown)
        {
            Mmio.Write32(group + Samda1PortLayout.OUTCLR_OFF, mask);
            Mmio.Write8(PinCfgAddress(pinNumber),
                (byte)(Samda1PortLayout.PINCFG0_INEN | Samda1PortLayout.PINCFG0_PULLEN));
            return;
        }
    ...
    {
        if ((Mmio.Read32(GroupBase(pinNumber) + Samda1PortLayout.DIR_OFF) & PinMask(pinNumber)) != 0u)
        {
            return PinMode.Output;
        }
        byte config = Mmio.Read8(PinCfgAddress(pinNumber));
        if ((config & (byte)Samda1PortLayout.PINCFG0_PULLEN) == 0)
        {
            return PinMode.Input;
        }
        uint level = Mmio.Read32(GroupBase(pinNumber) + Samda1PortLayout.OUT_OFF);
    ...
    {
        uint levels = Mmio.Read32(GroupBase(pinNumber) + Samda1PortLayout.IN_OFF);
        return (levels & PinMask(pinNumber)) != 0u ? PinValue.High : PinValue.Low;
    }

    protected override void Write(int pinNumber, PinValue value)
    {
        uint offset = (bool)value ? Samda1PortLayout.OUTSET_OFF : Samda1PortLayout.OUTCLR_OFF;
    ...
    {
        if (groupBase == Samda1Instances.PORTA_BASE) return (int)pin;
        if (groupBase == Samda1Instances.PORTB_BASE) return 32 + (int)pin;
    ...
    {
        return pinNumber < 32 ? Samda1Instances.PORTA_BASE : Samda1Instances.PORTB_BASE;
    }
    ...
    {
        return GroupBase(pinNumber) + Samda1PortLayout.PINCFG0_OFF + (uint)(pinNumber & 31);
    }
    ...
    {
#if LAMELLA_CORLIB_LINKED
        throw new System.NotSupportedException("SAMDA1 pin-change events are not implemented");
#else
        throw new System.Exception("SAMDA1 pin-change events are not implemented");
#endif
    }
