// A System.Device.Gpio driver for the Microchip SAML21, PA00..PA31 + PB00..PB31.
using System.Device.Gpio;
using Lamella.Generated;
using Lamella.Hardware;

public sealed class Saml21GpioDriver : GpioDriver
{
    ...
    {
        Mmio.Write32(GroupBase(pinNumber) + Saml21PortLayout.DIRCLR_OFF, PinMask(pinNumber));
    ...
    protected override void SetPinMode(int pinNumber, PinMode mode)
    {
        ...
        {
            Mmio.Write8(PinCfgAddress(pinNumber), (byte)Saml21PortLayout.PINCFG0_INEN);
            Mmio.Write32(group + Saml21PortLayout.DIRSET_OFF, mask);
            return;
        }

        Mmio.Write32(group + Saml21PortLayout.DIRCLR_OFF, mask);
        if (mode == PinMode.InputPullUp)
        {
            Mmio.Write32(group + Saml21PortLayout.OUTSET_OFF, mask);
            Mmio.Write8(PinCfgAddress(pinNumber),
                (byte)(Saml21PortLayout.PINCFG0_INEN | Saml21PortLayout.PINCFG0_PULLEN));
            return;
        }
        if (mode == PinMode.InputPullDown)
        {
            Mmio.Write32(group + Saml21PortLayout.OUTCLR_OFF, mask);
            Mmio.Write8(PinCfgAddress(pinNumber),
                (byte)(Saml21PortLayout.PINCFG0_INEN | Saml21PortLayout.PINCFG0_PULLEN));
            return;
        }
    ...
    {
        if ((Mmio.Read32(GroupBase(pinNumber) + Saml21PortLayout.DIR_OFF) & PinMask(pinNumber)) != 0u)
        {
            return PinMode.Output;
        }
        byte config = Mmio.Read8(PinCfgAddress(pinNumber));
        if ((config & (byte)Saml21PortLayout.PINCFG0_PULLEN) == 0)
        {
            return PinMode.Input;
        }
        uint level = Mmio.Read32(GroupBase(pinNumber) + Saml21PortLayout.OUT_OFF);
    ...
    {
        uint levels = Mmio.Read32(GroupBase(pinNumber) + Saml21PortLayout.IN_OFF);
        return (levels & PinMask(pinNumber)) != 0u ? PinValue.High : PinValue.Low;
    }

    protected override void Write(int pinNumber, PinValue value)
    {
        uint offset = (bool)value ? Saml21PortLayout.OUTSET_OFF : Saml21PortLayout.OUTCLR_OFF;
    ...
    {
        if (groupBase == Saml21Instances.PORTA_BASE) return (int)pin;
        if (groupBase == Saml21Instances.PORTB_BASE) return 32 + (int)pin;
    ...
    {
        return pinNumber < 32 ? Saml21Instances.PORTA_BASE : Saml21Instances.PORTB_BASE;
    }
    ...
    {
        return GroupBase(pinNumber) + Saml21PortLayout.PINCFG0_OFF + (uint)(pinNumber & 31);
    }
    ...
    {
#if LAMELLA_CORLIB_LINKED
        throw new System.NotSupportedException("SAML21 pin-change events are not implemented");
#else
        throw new System.Exception("SAML21 pin-change events are not implemented");
#endif
    }
