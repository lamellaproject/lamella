// A System.Device.Gpio driver for the Microchip SAMR30, PA00..PA31 + PB00..PB31 + PC00..PC31.
using System.Device.Gpio;
using Lamella.Generated;
using Lamella.Hardware;

public sealed class Samr30GpioDriver : GpioDriver
{
    ...
    {
        Mmio.Write32(GroupBase(pinNumber) + Samr30PortLayout.DIRCLR_OFF, PinMask(pinNumber));
    ...
    protected override void SetPinMode(int pinNumber, PinMode mode)
    {
        ...
        if (mode == PinMode.Output)
        {
            ...
            Mmio.Write8(PinCfgAddress(pinNumber), (byte)Samr30PortLayout.PINCFG0_INEN);
            Mmio.Write32(group + Samr30PortLayout.DIRSET_OFF, mask);
            return;
        }

        Mmio.Write32(group + Samr30PortLayout.DIRCLR_OFF, mask);
        if (mode == PinMode.InputPullUp)
        {
            Mmio.Write32(group + Samr30PortLayout.OUTSET_OFF, mask);
            Mmio.Write8(PinCfgAddress(pinNumber),
                (byte)(Samr30PortLayout.PINCFG0_INEN | Samr30PortLayout.PINCFG0_PULLEN));
            return;
        }
        if (mode == PinMode.InputPullDown)
        {
            Mmio.Write32(group + Samr30PortLayout.OUTCLR_OFF, mask);
            Mmio.Write8(PinCfgAddress(pinNumber),
                (byte)(Samr30PortLayout.PINCFG0_INEN | Samr30PortLayout.PINCFG0_PULLEN));
            return;
        }
    ...
    {
        if ((Mmio.Read32(GroupBase(pinNumber) + Samr30PortLayout.DIR_OFF) & PinMask(pinNumber)) != 0u)
        {
            return PinMode.Output;
        }
        byte config = Mmio.Read8(PinCfgAddress(pinNumber));
        if ((config & (byte)Samr30PortLayout.PINCFG0_PULLEN) == 0)
        {
            return PinMode.Input;
        }
        uint level = Mmio.Read32(GroupBase(pinNumber) + Samr30PortLayout.OUT_OFF);
    ...
    {
        uint levels = Mmio.Read32(GroupBase(pinNumber) + Samr30PortLayout.IN_OFF);
        return (levels & PinMask(pinNumber)) != 0u ? PinValue.High : PinValue.Low;
    }

    protected override void Write(int pinNumber, PinValue value)
    {
        uint offset = (bool)value ? Samr30PortLayout.OUTSET_OFF : Samr30PortLayout.OUTCLR_OFF;
    ...
    {
        if (groupBase == Samr30Instances.PORTA_BASE) return (int)pin;
        if (groupBase == Samr30Instances.PORTB_BASE) return 32 + (int)pin;
        if (groupBase == Samr30Instances.PORTC_BASE) return 64 + (int)pin;
#if LAMELLA_CORLIB_LINKED
    ...
    /// <summary>The four pads DS70005303C section 7.2 states cannot be configured as outputs,
    ...
    {
        if (pinNumber < 32) return Samr30Instances.PORTA_BASE;
        if (pinNumber < 64) return Samr30Instances.PORTB_BASE;
        return Samr30Instances.PORTC_BASE;
    }
    ...
    {
        return GroupBase(pinNumber) + Samr30PortLayout.PINCFG0_OFF + (uint)(pinNumber & 31);
    }
    ...
    {
#if LAMELLA_CORLIB_LINKED
        throw new System.NotSupportedException("SAMR30 pin-change events are not implemented");
#else
        throw new System.Exception("SAMR30 pin-change events are not implemented");
#endif
    }
