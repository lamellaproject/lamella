// A Lamella.Hardware.I2cDriver for the Nordic nRF52833 (TWI0, the LEGACY polled two-wire
using System;
...
public sealed class Nrf52833I2cDriver : I2cDriver
{
    ...
    public Nrf52833I2cDriver(Nrf52833TwiBinding binding)
    {
        uint twi = binding.TwiBase;
        _tasksStartRx = twi + Nrf52833TwiLayout.TASKS_STARTRX_OFF;
        _tasksStartTx = twi + Nrf52833TwiLayout.TASKS_STARTTX_OFF;
        _tasksStop = twi + Nrf52833TwiLayout.TASKS_STOP_OFF;
        _tasksResume = twi + Nrf52833TwiLayout.TASKS_RESUME_OFF;
        _eventsStopped = twi + Nrf52833TwiLayout.EVENTS_STOPPED_OFF;
        _eventsRxdReady = twi + Nrf52833TwiLayout.EVENTS_RXDREADY_OFF;
        _eventsTxdSent = twi + Nrf52833TwiLayout.EVENTS_TXDSENT_OFF;
        _eventsError = twi + Nrf52833TwiLayout.EVENTS_ERROR_OFF;
        _shorts = twi + Nrf52833TwiLayout.SHORTS_OFF;
        _intenclr = twi + Nrf52833TwiLayout.INTENCLR_OFF;
        _errorsrc = twi + Nrf52833TwiLayout.ERRORSRC_OFF;
        _enable = twi + Nrf52833TwiLayout.ENABLE_OFF;
        _pselScl = twi + Nrf52833TwiLayout.PSEL_SCL_OFF;
        _pselSda = twi + Nrf52833TwiLayout.PSEL_SDA_OFF;
        _rxd = twi + Nrf52833TwiLayout.RXD_OFF;
        _txd = twi + Nrf52833TwiLayout.TXD_OFF;
        _frequency = twi + Nrf52833TwiLayout.FREQUENCY_OFF;
        _address = twi + Nrf52833TwiLayout.ADDRESS_OFF;
        ...
        _bbSuspend = Nrf52833TwiLayout.SHORTS_BB_SUSPEND;
        _bbStop = Nrf52833TwiLayout.SHORTS_BB_STOP;
        _errAnack = Nrf52833TwiLayout.ERRORSRC_ANACK;
        _errDnack = Nrf52833TwiLayout.ERRORSRC_DNACK;
    }

    /// <summary>Brings the bound TWI up as a polled master: pins per the datasheet's Table
    /// 51 (the layout's PIN_CNF_TWI value), PSEL routed while disabled, every relevant
    /// register written explicitly (the shared-peripheral-ID rule), ENABLE = TWI last. The
    /// chip's FREQUENCY register is enumerated, so <paramref name="busHz"/> must be exactly
    /// 100000, 250000, or 400000 (realized 410.256 kbps) -- anything else throws.</summary>
    public override void Configure(int busHz)
    {
        uint frequency;
        if (busHz == 100000) frequency = Nrf52833TwiLayout.FREQUENCY_K100;
        else if (busHz == 250000) frequency = Nrf52833TwiLayout.FREQUENCY_K250;
        else if (busHz == 400000) frequency = Nrf52833TwiLayout.FREQUENCY_K400;
        else
        {
#if LAMELLA_CORLIB_LINKED
            throw new ArgumentException("nrf52833 twi rates are exactly 100000, 250000, or 400000");
#else
            throw new Exception("nrf52833 twi rates are exactly 100000, 250000, or 400000");
#endif
        }

        Mmio.Write32(_enable, Nrf52833TwiLayout.ENABLE_DISABLED);
        Mmio.Write32(_pinCnfSclReg, Nrf52833TwiLayout.PIN_CNF_TWI);
        Mmio.Write32(_pinCnfSdaReg, Nrf52833TwiLayout.PIN_CNF_TWI);
        ...
        Mmio.Write32(_enable, Nrf52833TwiLayout.ENABLE_TWI);
    }
    ...
    /// <summary>The block's write_then_read sequence: the write bytes go out with NO stop,
    /// then TASKS_STARTRX makes the hardware issue the REPEATED START for the read phase --
    /// the register-read shape the LSM303AGR capstone lives on.</summary>
