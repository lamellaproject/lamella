// A System.Device.Gpio driver for the Microchip SAML22, PA00..PA31 + PB00..PB31 + PC00..PC31.
using System.Device.Gpio;
using Lamella.Generated;
using Lamella.Hardware;

public sealed class Saml22GpioDriver : GpioDriver
{
    ...
    {
        Mmio.Write32(GroupBase(pinNumber) + Saml22PortLayout.DIRCLR_OFF, PinMask(pinNumber));
    ...
    protected override void SetPinMode(int pinNumber, PinMode mode)
    {
        ...
        {
            Mmio.Write8(PinCfgAddress(pinNumber), (byte)Saml22PortLayout.PINCFG0_INEN);
            Mmio.Write32(group + Saml22PortLayout.DIRSET_OFF, mask);
            return;
        }

        Mmio.Write32(group + Saml22PortLayout.DIRCLR_OFF, mask);
        if (mode == PinMode.InputPullUp)
        {
            Mmio.Write32(group + Saml22PortLayout.OUTSET_OFF, mask);
            Mmio.Write8(PinCfgAddress(pinNumber),
                (byte)(Saml22PortLayout.PINCFG0_INEN | Saml22PortLayout.PINCFG0_PULLEN));
            return;
        }
        if (mode == PinMode.InputPullDown)
        {
            Mmio.Write32(group + Saml22PortLayout.OUTCLR_OFF, mask);
            Mmio.Write8(PinCfgAddress(pinNumber),
                (byte)(Saml22PortLayout.PINCFG0_INEN | Saml22PortLayout.PINCFG0_PULLEN));
            return;
        }
    ...
    {
        if ((Mmio.Read32(GroupBase(pinNumber) + Saml22PortLayout.DIR_OFF) & PinMask(pinNumber)) != 0u)
        {
            return PinMode.Output;
        }
        byte config = Mmio.Read8(PinCfgAddress(pinNumber));
        if ((config & (byte)Saml22PortLayout.PINCFG0_PULLEN) == 0)
        {
            return PinMode.Input;
        }
        uint level = Mmio.Read32(GroupBase(pinNumber) + Saml22PortLayout.OUT_OFF);
    ...
    {
        uint levels = Mmio.Read32(GroupBase(pinNumber) + Saml22PortLayout.IN_OFF);
        return (levels & PinMask(pinNumber)) != 0u ? PinValue.High : PinValue.Low;
    }

    protected override void Write(int pinNumber, PinValue value)
    {
        uint offset = (bool)value ? Saml22PortLayout.OUTSET_OFF : Saml22PortLayout.OUTCLR_OFF;
    ...
    {
        if (groupBase == Saml22Instances.PORTA_BASE) return (int)pin;
        if (groupBase == Saml22Instances.PORTB_BASE) return 32 + (int)pin;
        if (groupBase == Saml22Instances.PORTC_BASE) return 64 + (int)pin;
    ...
    {
        if (pinNumber < 32) return Saml22Instances.PORTA_BASE;
        if (pinNumber < 64) return Saml22Instances.PORTB_BASE;
        return Saml22Instances.PORTC_BASE;
    }
    ...
    {
        return GroupBase(pinNumber) + Saml22PortLayout.PINCFG0_OFF + (uint)(pinNumber & 31);
    }
    ...
    {
#if LAMELLA_CORLIB_LINKED
        throw new System.NotSupportedException("SAML22 pin-change events are not implemented");
#else
        throw new System.Exception("SAML22 pin-change events are not implemented");
#endif
    }
